...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    /// How long to wait for the target app to become active before clicking
    /// anyway.
    static let activationTimeout: TimeInterval = 0.5

    /// How long to wait, after the target app becomes active, before clicking.
    /// The app needs a moment to make its window key.
    static let activationSettleDelay: TimeInterval = 0.02

    func bringToBackground() {
        NSApp.hide(self)
    }

    /// Hide Scoot, restore focus to the app that was frontmost when Scoot was
    /// invoked, and run `action` once that app is active.
    ///
    ...
    /// `acceptsFirstMouse(for:)`, which is false by default and which SwiftUI
    /// controls do not override. Browsers do override it, which is why a
    /// premature click still works in a browser but is swallowed elsewhere.
    ///
    /// `NSApp.hide(_:)` returns before the next app is active, so a click
    /// issued right after it can arrive while the target app is still
    /// inactive. The target app then eats the click, and the user has to press
    /// the key a second time.
    ///
    /// See: <https://github.com/mjrusso/scoot/issues/28>
    func bringToBackground(then action: @escaping () -> Void) {
        let target = currentApp

        bringToBackground()

        // Scoot itself is never a click target. If Scoot was frontmost when it
        // was invoked, there is no app to wait for.
        ...
        observer = NSWorkspace.shared.notificationCenter.addObserver(
            ...
            guard let app = notification.userInfo?[NSWorkspace.applicationUserInfoKey]
                  ...
                  app.processIdentifier == target.processIdentifier else {
                return
            }
        ...
        // Never drop the click, even if the app never reports itself active.
        DispatchQueue.main.asyncAfter(deadline: .now() + Self.activationTimeout) {
            if !hasRun {
                OSLog.main.log("Target app did not activate within \(Self.activationTimeout, privacy: .public)s; clicking anyway")
            }
            run()
        }
    }
